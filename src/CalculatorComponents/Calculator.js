...
class Calculator extends Component {
    ...
    let firstSpKeys = firstKeyArr.map((eachKey) => {
      return (
        <SpecialButton
          func={eachKey}
          key={eachKey}
          secondKeysClick={this.secondKeysClick}
        />
      );
    });
    return firstSpKeys;
    ...
    let secondSpKeys = functionsArr.map((eachKey) => {
      return (
        <Functions
          func={eachKey}
          key={eachKey}
          functionClick={this.functionClick}
        />
      );
    });
    return secondSpKeys;
  ...
  functionClick = (givenFunc) => {
    if (
      givenFunc === "del" &&
      document.getElementById("calcScreen").value.length > 0
    ) {
  ...
  addNumbers = () => {
    let funcs = numbersArr.map((eachKey) => {
      return (
        <Numbers func={eachKey} key={eachKey} numberClick={this.numberClick} />
      );
    });
    return funcs;
  };
    ...
    return (
      <div className="outerbody">
        <div className="calculatorBody">
          ...
          <div className="functions-container">{this.addFunctions()}</div>

          <div className="numbers-container">{this.addNumbers()}</div>
        </div>
      </div>
    );
  }
